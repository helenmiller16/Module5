...
* Basal-like group seems toseparate from other groups in all clustering techniques
* HER2-enriched separated in some techniques
* Luminal A and Luminal B similar
...
for(i in 1:K){
  j = i+1
  plot(Z[,i],Z[,j],pch=16,xlab=pclabs[i],ylab=pclabs[j],col=Cols(cdat$`ER-Status`))
}
legend(-45,0,pch=16,col=rainbow(5),levels(cdat$`ER-Status`))

# Let's see what features are associated with PC1
pc <- princomp(gdat)

loading1 <- V[, 1]
loadingOrder <- order(loading1)
loadingSorted <- loading1[order(loading1)]
loadingLabels <- colnames(gdat)[loadingOrder]

barplot(loadingSorted[1:10],cex.names=.6,main="PC 1 Loadings", )
loadingLabels[1:10]
```

1c - Overall, which dimension reduction method do you recommend for this data set and why?

* For visualization and separating groups, UMAP worked best. 
* PCA is best for exploring and understanding what's going on. 
* MDS and ICA didn't add anything useful
...
table(cdat$ER)
table(cdat$PR)
table(cdat$HER2) # hormonal receptor
table(cdat$Node) # number of lymph nodes with metastisis
table(cdat$Metastasis) # yes or no
table(cdat$ER,cdat$PR) # Progesterine receptor, (receptors)
...
```{r, echo = TRUE}
gdat.umap = umap(gdat)
plot(gdat.umap$layout[,1], y =gdat.umap$layout[,2], main = "UMAP", xlab = "UMAP1", ylab = "UMAP2", col = Cols(cdat$Subtype))
