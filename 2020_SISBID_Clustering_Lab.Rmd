...
* Problem 1a - We wish to plot the author texts as well as the words via a 2D scatterplot.  Which method would be best to use?  Why?
  * PCA because it allows you to perform dimredux on both features and observations 
...
pc1 <- pc$scores[, 1]
pc2 <- pc$scores[, 2]
plot(pc1, pc2, pch = 2, cex = .5, col = as.factor(rownames(author)))
```


* Problem 1c - Apply MDS to visualize the author texts.  Interpret the results.  
```{r}
Dmat = dist(author, method = "canberra") #Manhattan (L1) Distance
mdsres = cmdscale(Dmat,k=10)

i = 1; j = 2;
plot(mdsres[,i],mdsres[,j], main = "MDS for texts", col = as.factor(rownames(author)))
...
```{r}
Dmat = dist(t(author), method = "canberra") #Manhattan (L1) Distance
mdsres = cmdscale(Dmat,k=2)

i = 1; j = 2;
plot(mdsres[,i],mdsres[,j], main = "MDS for words", type = "n")
text(mdsres[, i], mdsres[, j], colnames(author))
...
ggplot() + 
    geom_point(data = Data,mapping = aes(x = pc1,y = pc2,color = PredLabel), pch = 16)  + 
    geom_point(gd,mapping = aes(x = pc1,y = pc2,color= factor(label)),size = 4) 
